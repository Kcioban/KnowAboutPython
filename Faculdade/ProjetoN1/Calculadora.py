...
def Fahrenheit():
    try:
        ...
        result_label.config(text=f"{valorC} graus Celsius é igual a {valorF} graus Fahrenheit")
    except ValueError:
...
def Celsius():
    try:
        ...
        result_label.config(text=f"{valorF} graus Fahrenheit é igual a {valorC} graus Celsius")
    except ValueError:
        result_label.config(text="Valor inválido")

convertApp = tk.Tk()
convertApp.title("Conversao de Graus")
convertApp.geometry("550x100")
convertApp.resizable(False, False)
convertApp.configure(bg="#808080")
...
result_label = tk.Label(convertApp, text="", font=("Inter", 12, "bold"), foreground="#000000", background="#808080")
result_label.grid(row=2, column=0, columnspan=500, padx=10, pady=5, sticky='w')
